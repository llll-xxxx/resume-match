...
function isProfessionalKeyword(label: string, evidence = "") {
  ...
  const contentWords = value.toLowerCase().match(/[a-z0-9+#]+/g)?.filter((word) => !["and", "or", "the", "a", "an", "of", "to", "for", "with", "in", "on", "into"].includes(word)) || [];
  if (!value || contentWords.length > 4) return false;
...
export async function POST(request: Request) {
  try {
    ...
    const { result, usage } = await generateJSON<AnalysisResult>({
      provider, customBaseUrl, apiKey, model, schema, schemaName: "resume_keyword_analysis", maxTokens: provider === "deepseek" ? 16_000 : 12_000,
      outputInstruction: "首次扫描只负责关键词提取和匹配分级。无论后续任务文字如何要求，本次所有关键词的 rewrites 必须返回空数组；红色关键词的完整 Bullet 改写将在用户打开该关键词时另行生成。KNOWN_KEYWORDS_DATA 之后最多补充 8 个新的高价值关键词。新增 label 必须是 JD 中连续出现的原文，不得删词、改写、概括或组合分散出现的词。排除公司业务描述、岗位称呼、宽泛对象、普通动作串和不适合作为简历检索词的整句职责。",
    ...
    const resultLimit = lockedKeywords.length || Math.min(48, knownKeywords.length + 8);
    const keywords = expandedResults.filter((item) => item && typeof item.label === "string" && Boolean(keywordEvidence(jd, item.label)) && isProfessionalKeyword(item.label, keywordEvidence(jd, item.label)) && typeof item.evidence === "string" && ["green", "yellow", "red"].includes(item.status) && ["must", "important", "supporting"].includes(item.importance)).slice(0, resultLimit).map((item, itemIndex) => {
